...
import zipkin.Codec;
import zipkin.Span;
import zipkin.server.elasticsearch.ESClient;
...
public class ZipkinQueryApiV1 {
  ...
  /** es业务日志所在的地址*/
  @Value("${zipkin.query.hosts}")
  String esLogHost = "";
  @Value("${zipkin.query.username}")
  String esLogUsername = "";
  @Value("${zipkin.query.password}")
  String esLogPassword = "";
  ...
  @RequestMapping(value = "/trace/logs", method = RequestMethod.GET, produces = APPLICATION_JSON_VALUE)
  public String getLogsByTraceId(
    @RequestParam(value = "serviceName", required = true) String serviceName,
    @RequestParam(value = "traceId", required = true) String traceId){//@RequestParam(value = "esUrl", required = true) String esUrl
    //System.out.println("esHost--:"+esLogHost+"--esUsername--:"+esLogUsername+"--esPassword--:"+esLogPassword+"--traceId--:"+traceId);
    ESClient esClient = new ESClient(esLogUsername,esLogPassword);
    List<String> logs = esClient.getLogs(esLogHost, serviceName, traceId);
    if(logs==null){
      //throw new LogsNotFoundException(serviceName,traceId);
      logs=new ArrayList<>();
      logs.add("none");
    }
    JsonAdapter<List> jsonAdapter = new Moshi.Builder().build().adapter(List.class);
    return jsonAdapter.toJson(logs);
    //return new String(Codec.JSON.writeSpans(trace), UTF_8);
  }
