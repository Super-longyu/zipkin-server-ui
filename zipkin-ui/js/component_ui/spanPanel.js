...
import TraceEsLogsData from '../component_data/traceEsLogs';
import EsLogsPanelUI from '../component_ui/esLogsPanel';
...
export default component(function spanPanel() {
  ...
  this.after('initialize', function() {
    ...
    TraceEsLogsData.attachTo(document);
    this.on(document, 'uiRequestSpanPanel', this.show);
    this.on(document, 'traceEsLogsPageModelView', function (ev, data){
      EsLogsPanelUI.attachTo('#esLogsPanel');
      this.$node.find('#traceEsLogsLink').click(e => {
        e.preventDefault();
        this.trigger('uiRequestEsLogsPanel', {
          title: `Trace logs of ${this.attr.traceId}`,
          obj: data.traceEsLogs,
        });
      });
